...
public fun charge<T>(account: &mut Account<T>, mandate: &mut Mandate<T>, amount: u64, clock: &Clock, ctx: &mut TxContext) {
    ...
    settle(account, mandate, amount, ctx);
}
...
/// 资金守恒逻辑的唯一出口。
fun settle<T>(account: &mut Account<T>, mandate: &mut Mandate<T>, amount: u64, ctx: &mut TxContext) {
    ...
    transfer::public_transfer(paid, mandate.merchant);
    event::emit(Charged {
        mandate_id: object::id(mandate), account_id: mandate.account_id,
