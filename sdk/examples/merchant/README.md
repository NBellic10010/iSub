...
## 多语言
非 Node 后端:直接对 `POST {BASE_URL}/usage`(头 `x-isub-api-key` + `x-isub-mandate`,体 `{amount, usageId}`)发请求,webhook 按同样的 HMAC 方案验签即可。`@isubpay/sdk/client` 只是 Node 的便捷封装。
